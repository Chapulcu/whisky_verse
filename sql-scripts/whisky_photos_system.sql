...
RETURNS TRIGGER AS $$
BEGIN
    -- photos_taken sayısını artır
    PERFORM update_user_activity(NEW.user_id, 'photo_taken', 1);
    RETURN NEW;
END;
...
-- Gamification tablosundaki achievements.ts ile uyumlu olması için
-- photos_taken alanının trigger ile güncellendiğinden emin ol

COMMENT ON TABLE whisky_photos IS 'Kullanıcıların yüklediği viski fotoğrafları';
COMMENT ON COLUMN whisky_photos.is_approved IS 'Admin onayı (şimdilik otomatik true)';
COMMENT ON COLUMN whisky_photos.is_primary IS 'Viski için ana fotoğraf mı?';